...
def prepare_centernet_targets(
    ...
    #
    Fixed version - Properly handles numpy to torch conversion
    ...
    Args:
        ...
        gaussian_overlap: Minimum overlap for Gaussian radius calculation
        min_radius: Minimum Gaussian radius
    ...
    heatmap = torch.zeros(B, num_classes, H, W, device=device, dtype=torch.float32)
    offset = torch.zeros(B, 2, H, W, device=device, dtype=torch.float32)
    size = torch.zeros(B, 3, H, W, device=device, dtype=torch.float32)
    rot = torch.zeros(B, 2, H, W, device=device, dtype=torch.float32)
    vel = torch.zeros(B, 2, H, W, device=device, dtype=torch.float32)
    ...
    # Target values
    target_offset = torch.zeros(B, max_objects, 2, device=device, dtype=torch.float32)
    target_size = torch.zeros(B, max_objects, 3, device=device, dtype=torch.float32)
    target_rot = torch.zeros(B, max_objects, 2, device=device, dtype=torch.float32)
    target_vel = torch.zeros(B, max_objects, 2, device=device, dtype=torch.float32)
    
    # Compute BEV voxel size
    ...
    for b in range(B):
        ...
        num_objs = min(len(gt_boxes_np), max_objects)
        
        for k in range(num_objs):
            box = gt_boxes_np[k]
            cls_id = int(gt_labels_np[k])
            ...
            pixel_x = (x - x_min) / voxel_size_x
            pixel_y = (y - y_min) / voxel_size_y
            ...
            ct_int = np.array([int(pixel_x), int(pixel_y)], dtype=np.int32)
            
            # Fractional offset
            ct_offset = np.array([pixel_x - ct_int[0], pixel_y - ct_int[1]], dtype=np.float32)
            
            # Box size in pixels
            box_w_pixel = w / voxel_size_x
            box_l_pixel = l / voxel_size_y
            
            # Calculate Gaussian radius
            radius = gaussian_radius((box_l_pixel, box_w_pixel), min_overlap=gaussian_overlap)
            radius = max(min_radius, int(radius))
            
            # Draw Gaussian
            heatmap_np = heatmap[b, cls_id].cpu().numpy()
            draw_gaussian(heatmap_np, ct_int, radius)
            heatmap[b, cls_id] = torch.from_numpy(heatmap_np).to(device)
            
            # Store indices and masks
            ind[b, k] = ct_int[1] * W + ct_int[0]
            mask[b, k] = 1
            reg_mask[b, k] = 1
            
            # ✅ FIX: Convert to torch tensors before assignment
            # Offset target
            target_offset[b, k, 0] = float(ct_offset[0])
            target_offset[b, k, 1] = float(ct_offset[1])
            offset[b, 0, ct_int[1], ct_int[0]] = float(ct_offset[0])
            offset[b, 1, ct_int[1], ct_int[0]] = float(ct_offset[1])
            
            # Size target
            target_size[b, k, 0] = float(w)
            target_size[b, k, 1] = float(l)
            target_size[b, k, 2] = float(h)
            size[b, 0, ct_int[1], ct_int[0]] = float(w)
            size[b, 1, ct_int[1], ct_int[0]] = float(l)
            size[b, 2, ct_int[1], ct_int[0]] = float(h)
            
            # Rotation target
            sin_yaw = float(np.sin(yaw))
            cos_yaw = float(np.cos(yaw))
            target_rot[b, k, 0] = sin_yaw
            target_rot[b, k, 1] = cos_yaw
            rot[b, 0, ct_int[1], ct_int[0]] = sin_yaw
            rot[b, 1, ct_int[1], ct_int[0]] = cos_yaw
            
            # Velocity target (if available)
            if len(box) > 7:
                vx = float(box[7]) if len(box) > 7 else 0.0
                vy = float(box[8]) if len(box) > 8 else 0.0
                target_vel[b, k, 0] = vx
                target_vel[b, k, 1] = vy
                vel[b, 0, ct_int[1], ct_int[0]] = vx
                vel[b, 1, ct_int[1], ct_int[0]] = vy
