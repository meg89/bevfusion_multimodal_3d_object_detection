...
class NuScenesDataset(Dataset):
    ...
    def __init__(
        ...
        # Load pickle file
        pkl_path = self.data_root / f'nuscenes_infos_{split}.pkl'
        ...
        print(f"Loaded {len(self.infos)} samples for {split} split")
...
def main():
    ...
    config = {
        ...
        'fusion_type':  'attention', #'late', #'bev',  # 'bev', 'attention', 'late'
        'detection_head': 'centernet',  # 'centernet', 'anchor'
        'num_classes': 10,
...
def inference(
    ...
    from fusion_detection import MultiModal3DDetector
    model = MultiModal3DDetector(
        num_classes=config['num_classes'],
        fusion_type=config['fusion_type'],
        detection_head=config['detection_head']
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'train':
        ...
        print("Usage:")
        print("  python train_detection.py train          # Train model")
