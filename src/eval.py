...
def evaluate(
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        #Diagnostic: 
        cam_features = model.camera_encoder(camera_imgs)
        print(f"Camera features: mean={cam_features.mean():.3f}, std={cam_features.std():.3f}")
        if cam_features.std() < 0.01:
            print("❌ PROBLEM: Camera encoder producing constant features!")
            print("→ Check if encoder is frozen")

        lidar_features = torch.stack([model.lidar_encoder(p.to(device)) for p in lidar_points])
        bev_features = model.fusion(cam_features, lidar_features, None)
        print(f"BEV shape: {bev_features.shape} (expected: [B, 256, 200, 200])")
        if bev_features.shape[-2:] != (200, 200):
            print("❌ PROBLEM: Wrong BEV size!")
        
        # Check 3: Detection outputs
        predictions = model.detection_head(bev_features)
        print(f"Size predictions: mean={predictions['size'].mean():.3f}, std={predictions['size'].std():.3f}")
    
        if predictions['size'].mean() < 0.1:
            print("❌ PROBLEM: Size head predicting tiny values!")
            print("→ Check size head initialization")
        ...
        if trainable < total * 0.5:
            print("❌ PROBLEM: Most parameters frozen!")
...
def main(config_path: Optional[str] = None):
    ...
    save_and_print_metrics(val_metrics, "eval_metrics_output.txt" )
